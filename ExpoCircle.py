...
class ExpoCircle(Drawer):
    def __init__(self, size=Coords2D(1920, 800), length=60, rate=1, field_size=Coords2D(100, 100), border=False, points_no=10, power = 2, iters = 40):
        ...
        self.file_name = "videos/ExpoCircle_" + str(self.power) + "_"
    ...
    def draw_image(self, state, size=None):
        ...
        for i in range(1,state+1):
            ...
            end =  self.offset_point(self.point(int(math.pow(i,self.power)) % self.points_no))
